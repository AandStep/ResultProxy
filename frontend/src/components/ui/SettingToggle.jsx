...
export const SettingToggle = ({ title, description, isOn, onToggle }) => {
  return (
    ...
    >
      ...
      <div
        className={`relative w-14 h-8 rounded-2xl transition-colors duration-300 ease-in-out shrink-0 ${isOn ? "bg-[#007E3A]" : "bg-zinc-700"}`}
      >
        <div
          className={`absolute top-1 left-1 bg-white w-6 h-6 rounded-full transition-transform duration-300 ease-in-out ${isOn ? "transform translate-x-6" : ""}`}
        ></div>
      </div>
    </div>
  );
