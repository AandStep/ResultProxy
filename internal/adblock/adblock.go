...
// Fallback ad domains used before full lists are loaded.
...
// List sources for domain-based ad blocking.
var listURLs = []string{
	"https://pgl.yoyo.org/adservers/serverlist.php?hostformat=nohtml&showintro=0",
}

// Blocker provides DNS-based ad blocking via domain lists.
// Unlike the Node.js version (which used @ghostery/adblocker ~40MB),
// this uses compact domain lists that integrate with sing-box DNS rules.
type Blocker struct {
	mu      sync.RWMutex
	domains map[string]struct{} // set of blocked domains
	loaded  bool
}

// New creates a Blocker with fallback domains.
func New() *Blocker {
	b := &Blocker{
...
// LoadFromCache loads cached domain lists from disk.
...
// UpdateLists downloads fresh ad domain lists and caches them.
...
// IsAdDomain checks if a hostname is an ad/tracking domain.
func (b *Blocker) IsAdDomain(hostname string) bool {
	...
	// Suffix match: "ads.example.com" matches "example.com" in the list.
	for d := range b.domains {
...
// GetDomainCount returns the number of blocked domains.
...
// GetDomains returns all blocked domains as a sorted slice.
// Useful for generating sing-box DNS rules.
