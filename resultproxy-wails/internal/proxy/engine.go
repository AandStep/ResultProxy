...
func buildOutbounds(proxy ProxyConfig) []SBOutbound {
	...
	// Proxy outbound — depends on type.
...
func buildProxyOutbound(proxy ProxyConfig) SBOutbound {
	...
	case "ss", "shadowsocks":
		...
		if proxy.Extra != nil {
			...
			if json.Unmarshal(proxy.Extra, &extra) == nil && extra.Method != "" {
				method = extra.Method
			}
		}
		return SBOutbound{
...
func buildDNS(cfg EngineConfig) *SBDNS {
	...
	// In proxy mode, use local DNS. In tunnel mode, use encrypted DNS.
	if cfg.Mode == ProxyModeTunnel {
		dns.Servers = []SBDNSServer{
			{Tag: "google", Address: "https://dns.google/dns-query"},
